...
export const candle = (columns, rows, settings, status) => {
  ...
  const legend =
    legendVisible && getCandleLegend({ showMA, MA, legendName, labelMap });
  const tooltip =
    tooltipVisible && getCandleTooltip({ metrics, dataType, digit, labelMap });
  const visualMap =
    showVol && getCandleVisualMap({ downColor, upColor, MA, showMA });
